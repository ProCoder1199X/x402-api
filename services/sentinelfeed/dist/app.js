...
app.get("/api/v1/events", (request, response) => {
    ...
    if (!topic || topic.length > 120)
        return response.status(400).json({ error: "topic is required and must be at most 120 characters" });
    if (!["standard", "realtime"].includes(tier))
        return response.status(400).json({ error: "tier must be standard or realtime" });
    if (Number.isNaN(Date.parse(since)))
        return response.status(400).json({ error: "since must be an ISO-8601 timestamp" });
    const digest = (0, node_crypto_1.createHash)("sha256").update(`${topic.toLowerCase()}:${since}:${tier}`).digest("hex");
...
app.get("/api/v1/entities/:entityId/mentions", (request, response) => {
    ...
    if (!entityId || !/^[-A-Za-z0-9_:.]{1,100}$/.test(entityId))
        return response.status(400).json({ error: "entityId is invalid" });
    if (!/^\d+[mhd]$/.test(window))
        return response.status(400).json({ error: "window must look like 15m, 1h, or 1d" });
    const digest = (0, node_crypto_1.createHash)("sha256").update(`${entityId}:${window}`).digest("hex");
