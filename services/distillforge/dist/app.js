...
app.post("/api/v1/infer/classify", (request, response) => {
    ...
    if (!text || !taxonomy)
        return response.status(400).json({ error: "text and taxonomy are required" });
    const digest = (0, node_crypto_1.createHash)("sha256").update(`${taxonomy}:${text}`).digest("hex");
...
app.post("/api/v1/infer/extract", (request, response) => {
    ...
    if (!document || !schema || typeof schema !== "object" || Array.isArray(schema))
        return response.status(400).json({ error: "document and an object schema are required" });
    const digest = (0, node_crypto_1.createHash)("sha256").update(document).digest("hex");
...
app.post("/api/v1/infer/reason", (request, response) => {
    ...
    if (input === "{}")
        return response.status(400).json({ error: "a JSON reasoning payload is required" });
